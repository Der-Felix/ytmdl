...
// Case B: release artifact generator uses Schema 10, latest target is Schema 12 -> EXPECT qualification FAIL
func TestReleaseConsistency_CaseB_SchemaMismatch(t *testing.T) {
	...
	outStr := string(out)
	if !strings.Contains(outStr, "does not match latest DB migration schema (12)") &&
		!strings.Contains(outStr, "RELEASE CONSISTENCY ERROR") {
