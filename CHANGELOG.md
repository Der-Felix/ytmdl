...
- Tracked separately: a failed item's staging directory is still not removed, and an item whose 30-minute track timeout passes — also while waiting for the session slot — ends as `cancelled` without a retry.
